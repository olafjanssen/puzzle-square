...
define(['jquery', 'amplify', 'messages', 'card'], function ($, eventBus, Messages, Card) {
    'use strict';
    ...
    function onGridFilled() {
        ...
        setTimeout(function () {
            $('.grid-cell').each(function () {
                ...
                $(this).css({transform: 'translate3d(0px,' + (2 * window.innerHeight + offset) + 'px, 0) rotate(' + rotation + 'deg)'});
            });
        }, 2000);
    }
    ...
    function render(cols, rows, traitDirections) {
        ...
        $('.grid-cell').each(function () {
            ...
            $(this).css({transform: 'translate3d(' + Math.cos(phi) * rho * window.innerWidth + 'px,' + Math.sin(phi) * rho * window.innerHeight + 'px,0) rotate(' + rot + 'deg)'});
        });
        // animate back again
        setTimeout(function () {
            $('.grid-cell').each(function () {
